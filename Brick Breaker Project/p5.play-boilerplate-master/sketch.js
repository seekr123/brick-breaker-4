...
function setup() {
  ...
  for(var i=120;i<displayHeight-40;i+=110){
    vhbrick2.push(new Brick(displayWidth-120,i,'white',vhbrick1img))
  }
  ...
  for(var i=230;i<displayHeight-100;i+=110){
    hbrick2.push(new Brick(displayWidth-280,i,'lightBlue',hbrick1img))
  }
  ...
  for(var i=340;i<displayHeight-220;i+=110){
    brick2.push(new Brick(displayWidth-440,i,'orange',brick1img))
  }
  for(var i=395;i<displayHeight-280;i+=110){
    brick3.push(new Brick(displayWidth-520,i,'orange',brick1img))
  }
...
function draw() {
  ...
  for(var i=0;i<vhbrick1.length;i++){
    ...
    if(detectCollision(ball,vhbrick1[i])){      
      ...
      switch(vhbrick1State)
      {
        
        case 3:vhbrick1[i].changeImage("img",hbrick1img);
               vhbrick1State-=1;
               break;
        case 2:vhbrick1[i].changeImage("img",brick1img);
               vhbrick1State-=1;
               break;
       case 1:vhbrick1[i].destroy();
  ...
  for(var i=0;i<vhbrick2.length;i++){
    ...
    if(detectCollision(ball,vhbrick2[i])){      
      ball.velocityX=random(-8,-12);
      ball.velocityY=random(-8,-12);
      switch(vhbrick2State)
      {
        
        case 3:vhbrick2[i].changeImage("img",hbrick1img);
               vhbrick2State-=1;
               break;
        case 2:vhbrick2[i].changeImage("img",brick1img);
               vhbrick2State-=1;
               break;
       case 1:vhbrick2[i].destroy();
  ...
  for(var i=0;i<hbrick1.length;i++){
    ...
    if(detectCollision(ball,hbrick1[i])){      
      ...
      switch(hbrick1State)
      {
        case 2: hbrick1[i].changeImage("img",brick1img);
                hbrick1State-=1;
               break;
        case 1:hbrick1[i].destroy();
  ...
  for(var i=0;i<hbrick2.length;i++){
    ...
    if(detectCollision(ball,hbrick2[i])){      
      ...
      switch(hbrick2State)
      {
        case 2: hbrick2[i].changeImage("img",brick1img);
                hbrick1State-=1;
               break;
        case 1:hbrick2[i].destroy();
  ...
  fill('black');
  text('score: '+score,displayHeight/30,displayWidth/1.5);
